...
#sum(dot(ntemp,gtemp)^(delta-1.)))
#n n[gmin_re:j],(gam[gmin_re:j])^(delta-1.))))                                                            
   end
   n_re[j]=nre  
end
...
#nno=SharedArray{Float64}(ngamma)                                                                                  
 ...
 #                                                                                                                  
  ...
  if shock >= 1 && m >= mthr   #injection of power-law spectrum of particles                                       
   ...
   tacc=inv(vshock*inv(3000.0))*3.0e7*2.4e4*sqrt(delta*inv(beff)) #..acceleration time from Kang 2012 Eq\

@inbounds    gam_c=gam[ngamma]

@fastmath @simd   for i in 1:ngamma   #finds maximum gamma where tacc < tlosses                     
   ic_lose=b1*((gam[1]^2.))*zz4
@inbounds   diff=(gam[i]-1.0)*cons1 # in cm^2/s                                                                             
@inbounds    sh_gain=(inv(f)*gam[i]*(vpre)^2.*cons2*inv(3.*diff))

          if ic_lose > sh_gain
          gam_c=gam[i]
           end
               end
   inte=(-delta+2.0)*inv(((gam_c)^(-delta+2.0)-(g_min)^(-delta+2.0)))
  Ke=((10.^ecr)*inv(1e6*erest*evtoerg))*inv(inte)

@inbounds @simd    for j in 1:ngamma#eachindex(n_inj)
...
function eth(n::Float64,volume::Float64,t::Float64)
 ethermal=log10(n/mp)+volume+log10(1.5*kb*t)
return ethermal
end

function ecri(n::Float64,vshock::Float64,volume::Float64)
  Ecr_inj=15.0+log10(fi)+log10(n/mp*vshock^3.0)+log10(mp)+0.6666*volume
return Ecr_inj
end
...
#@fastmath ethermal=log10(p[i,7]/mp)+volume+log10(1.5*kb*p[i,8])  #...observed                                                                                  
 ...
 m=p[9,i]
  ...
  tpre=p[8,i]*(16.*m*m)/((5.*m*m-1)*(m*m+3))                                                                                                                  
  cspre=1e-5*sqrt(1.666*tpre*kb*inv(mp*1.1))    #...preshock                                                                             
