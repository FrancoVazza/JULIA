...
@everywhere include(string(main,"radio.jl")) #synchrotron radio power
...
#@everywhere include(string(main,"read_HB.jl"))
...
@inbounds        @simd    for i in 1:s2-s1+1 
  ...
  @views       map[:,:,13]+=sz_dec[:,:,i]
   ...
   @views      map[:,:,13]+=sz_dec[:,i,:]
    ...
    @inbounds        @simd    for i in 1:s2-s1+1
    @views     map[:,:,1]+=d[i,:,:] #av.density map                                       
    @views     map[:,:,14]+=dm[i,:,:]  #av. temperature map                                 
    @views     map[:,:,10]+=(lx1[i,:,:]+lx2[i,:,:])
    @views     map[:,:,11]+=lx1[i,:,:]
    @views     map[:,:,12]+=lx2[i,:,:]
    @views     map[:,:,13]+=sz_dec[i,:,:]
...
#            toc()
 ...
 fileb="mach_psi_table.txt"
 file=string(main,fileb) #"mach_psi_table.txt")                             
 psi_f=readdlm(file)
...
#...................selection from the HB efficiency function for Psi(M)
to=t1/keV
@inbounds @simd for ii in 1:nt-1 
...
msel=n33
    ...
    psi=psi_f[msel,tsel]# fsel(tsel,msel)
    xsi=1e-2*psi#_sel(m,t1/keV,ff,mpsi)                         
    if xsi <=2e-5
    xsi=2e-5
    end
        prad=radio(d1,t1,m1,bx1,by1,bz1,nu,pradio1,psi)
        pradio3D[ta]=prad             #....HB + reacc.
        pradio3D2[ta]=prad*1e-4/(xsi) #....fixed accel.efficiency
  #    println(pradio3D[ta]," ",pradio3D2[ta], " ",psi," ",xsi)
   end
  # println("radio")
  #          toc()
      rm=similar(d)
...
######################################################
	...
	for lll in 1:2   #selects a line of sight (Z is faster, X is slower
tic()
    	    if lll == 1
        @everywhere los="Z"
	    end
...
@views    map_total[:,:,ipradio1]+=1e-30
@views    map_total[:,:,ipradio2]+=1e-30
...
filep1=string(root,"maps/map_all",mod,"_",snap,"_1024_",los,"newHB2.fits")
 f = FITS(filep1, "w");
write(f,map_total)
close(f)
toc()
