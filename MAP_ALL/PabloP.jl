...
#...each processor reads the input files file1,file2 in parallel, i.e. each processor only reads a part of the entire domain, depending on the bounds defined above.
  ...
  freq_SZ=220e9 #...GHz (SPT-like)
  SZ(d,t,freq_SZ,sz_dec)
...
@simd for i in 1:n
    map_total[i,j,itmw]=map_total[i,j,itm1]/(map_total[i,j,id]) 
 end
 end
...
filep1=string(root,"map_all",mod,"_",snap,"_1024_",los,"new.fits")
 f = FITS(filep1, "w");
write(f,map_total)
close(f)
